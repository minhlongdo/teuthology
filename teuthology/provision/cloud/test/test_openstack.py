...
class TestOpenStackProvisioner(TestOpenStackBase):
    ...
    def test_read_conf(self, input_conf):
        ...
        for topic in ['machine', 'volumes']:
            if topic in input_conf:
                assert obj.conf[topic] == input_conf[topic]
            else:
                assert obj.conf[topic] == config.openstack[topic]

    def test_read_conf_legacy(self):
        input_conf = dict(openstack=[
            dict(machine=dict(disk=1, ram=2, cpu=3)),
            dict(volumes=dict(count=1, size=1)),
        ])
        obj = self.get_obj(conf=input_conf)
        assert obj.conf['machine'] == dict(disk=1, ram=2, cpu=3)
        assert obj.conf['volumes'] == dict(count=1, size=1)
    ...
    def test_size(self, input_attrs, func_or_exc):
        size_attrs = [
            dict(ram=1, disk=9999, vcpus=99),
            dict(ram=2**16, disk=1, vcpus=99),
